  ...
  try {
    ...
    if (typeof request.headers?.entries === 'function') {
      ...
      for (const key in request.headers) {
        forwardHeader(key, request.headers[key])
      }
    }

    // 如果缺少Authorization，尝试从请求体参数中获取
    if (!requestHeaders.authorization && request.query?.username && request.query?.password) {
      const auth = Buffer.from(`${request.query.username}:${request.query.password}`).toString('base64')
      requestHeaders.authorization = `Basic ${auth}`
      console.log('[PROXY] 从查询参数构造Authorization头')
    }
